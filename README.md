...
- **Mob Actor Type, Monster Statblocks & Compendium**:
  ...
  - **Unique Mob Attributes & Sheet Display**: Dedicated fields for `Treasure` drops, `XP` defeat rewards, `Classification` (Mob, Elite, Boss), `Creature Type` (Animal, Humanoid, Ooze, Monstrous, etc.), `Floor & Location`, `Source Citation`, `Surprise Difficulty` (e.g. `11+F`), and `Evade Difficulty` (e.g. `12+F` based on $10 + \text{DEX Mod} + \text{Floor Number}$).
